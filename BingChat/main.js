/**
 * @title BingChat
 * @create_at 2023-07-28 12:00:00
 * @rule ^([Bb]i[nm]g) ([\s\S]+)$
 * @description 必应聊天AI机器人&AI绘画插件，输入'bing ?'进行与BingChat互动，具体见代码注释。
 * @author Masato
 * @version v1.0.0
 * @public false
 * @form {key: "BingChat.Token", title: "Bing ChatToken", tooltip: "设置BingChat Token(可免Token使用，如后台日志出现CAPTCHA字样，请加上Token)" }
 * @form {key: "Bimg.Token", title: "Bimg Token", tooltip: "设置Bimg Token（如需使用AI绘画功能，请务必设置此项）" }
 * @class AI
 * @icon https://th.bing.com/th?id=ODLS.87518fab-8fc1-4893-a7f1-c7d88283a35a&w=32&h=32&qlt=92&pcl=fffffa&o=6&pid=1.2
 */
/* 
使用命令'set BingChat Token ?'设置BingChat Token(可免Token使用，如后台日志出现CAPTCHA字样，请加上Token)
使用命令'set Bimg Token ?'设置Bimg Token（如需使用AI绘画功能，请务必设置此项）
...
*/
const { sender: s, Bucket: BncrDB, sleep } = require("sillygirl");
...
(async () => {
    ...
    let query = await s.param(2);
    if (await s.param(1) === 'bimg' || await s.param(1) === 'Bimg') {
        ...
        try {
            ...
            let imageLinks = await generateImagesLinks(query);
            if (imageLinks) {
                for (let i = 0; i < num; i++) {
                    let url = imageLinks[i];
                    s.reply(`[CQ:image,file=${url}]`);
                    await sleep(1000);
                }
                return;
            }
    ...
    let platform = await s.getPlatform(),
        ...
        suggestion = {};
    if (query === '清空上下文') {
        await BingChatStorage.set(`${platform}_${userId}`, "");
    ...
    /* 获取上下文 */
    const getUesrInfo = await BingChatStorage.get(`${platform}_${userId}`);
    if (getUesrInfo) {
