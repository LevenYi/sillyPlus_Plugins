...
function pushMsg(msg, platform, uid, cid) {
    // let s = require("SillyGirl").newSender({
    //     platform: platform,
    //     user_id: uid,
    //     chat_id: cid,
    // })
...
//tg快捷复制
function toQuickCopy(imtype, title, msg) {
    if (imtype == "pgm")
        return "【**" + title + "**】`" + msg + "`"
    else if (imtype == "tg")
        return "【*" + title + "*】`" + msg + "`"
...
async function getBind(imtype, uid) {
    let db = new Bucket("pin" + imtype.toString().toUpperCase())
    let allpins = await db.keys();
    let pin = []
    for (let i = 0; i < allpins.length; i++)
        if (await db.get(allpins[i]) == uid)
            pin.push(allpins[i])
    return pin
}

//获取绑定pin的用户
async function getBindByPin(pin) {
    let types = ["qq", "wx", "tg"]	//平台
    let users = []
    types.forEach(async type => {
        let db = new Bucket("pin" + type.toUpperCase())
        let id = await db.get(pin)
        if (id) {
            users.push({ type, id })
        }
    })
...
function notifyMasters(msg) {
    let types = ["qq", "tg", "wx"]
    types.forEach(type => {
        let matsers = await(new Bucket(type)).get("masters")
        if (matsers)
            matsers.split("&").forEach(id => pushMsg(msg, type, id))
    })
}

//在totype平台的群cid中向绑定京东账号pin的用户通知msg
function notifyPinInGroup(totype, cid, pin, msg) {
    let uid = await(new Bucket("pin" + totype.toUpperCase())).get(pin)
    if (uid) {
        pushMsg(msg, totype, uid, cid)
        return true
    }
    else
        return false
...
function notifyPin(pin, msg) {
    ...
    types.forEach(async type => {
        ...
        let uid = await binds.get(pin) || await binds.get(encodeURI(pin))
        if (uid)	//name为pin
    ...
    if (!to.length)
        ...
        to.forEach(user => pushMsg(msg, user.type, user.id))
    return to
}
...
class JD {
    ...
    async isLogin() {
        try {
            let resp = await fetch("https://plogin.m.jd.com/cgi-bin/ml/islogin", {
                headers: this.headers
            })
            let data = await resp.json()
            //console.log(JSON.stringify(data))
            return Number(data.islogin)
        }
        catch (e) {
            console.log(e)
            return null
    ...
    async userInfo() {
        try {
            let resp = await fetch("https://me-api.jd.com/user_new/info/GetJDUserInfoUnion", {
                headers: this.headers
            })
            let data = await resp.json()
            return data.data
    ...
    async beanInfo1(days) {
        let page = 1	//京豆收入页
        let info = []  //每一项收入活动详情记录
        let limit = 100 //死循环保险
        let day = ""  //记录前一项收入的日期，以统计记录天数
        let count = 0	//天数统计
        const url = "https://api.m.jd.com/client.action?functionId=getJingBeanBalanceDetail";
        while (limit-- > 0) {
            ...
            try {
                let data = await resp.json()
                if (data.code == 0) {
                    for (let income of data.detailList) {
                        let infoday = income.date.match(/\d+(?= )/)[0]	//收入项时间-天
                        if (day != infoday) {
                            day = infoday
                            if (count == days)
                                return info
    ...
    async beanInfo2(days) {
        ...
        let count = 0	//天数统计
        while (limit-- > 0) {
            const url = "https://bean.m.jd.com/beanDetail/detail.json?page=" + page;
            let resp = await fetch(url, {
                headers: this.headers
            })
            try {
                let data = await resp.json()
                for (let income of data.jingDetailList) {
                    let infoday = income.date.match(/\d+(?= )/)[0]	//收入项时间-天
                    if (day != infoday) {
                        day = infoday
                        if (count == days)
    ...
    async expireBeans() {
        const url = `https://wq.jd.com/activep3/singjd/queryexpirejingdou?_=${Date.now()}& g_login_type=1 & sceneval=2`;
        let resp = await fetch(url, {
            headers: {
                "Cookie": this.cookie,
        ...
        try {
            return JSON.parse((await resp.text()).slice(23, -13)).expirejingdou
        }
        catch (err) {
            return null
        }
    ...
    async redPackets() {
        const url = `https://m.jingxi.com/user/info/QueryUserRedEnvelopesV2?type=1&orgFlag=JD_PinGou_New&page=1&cashRedType=1&redBalanceFlag=1&channel=1&_=${+new Date()}& sceneval=2 & g_login_type=1 & g_ty=ls`;
...
/*************第三方私人api******************/
async function NolanServer(server, body) {
    const url = "https://api.nolanstore.cc/" + server;
    let resp = await fetch({
        method: "post",
        body: body
    })
...
//京东口令解析-nolan
function JComExchange(code) {
    return NolanServer("JComExchange", { "code": code })
}

//链接转口令-noaln
function JCommand(url, title, image) {
...
class Telegram {
    constructor(token, proxy) {
        this.token = token
        this.url = proxy ?? "https://api.telegram.org" + "/bot" + token
    }
