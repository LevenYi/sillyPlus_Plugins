...
function main() {
    const qb = Bucket("qbittorrent")
    const host = qb.get("addr")
    const uname = qb.get("username")
    const pwd = qb.get("password")
    //console.log([host,uname,pwd].join("\n"))
    if (!host || !uname || !pwd) {
        let tip = "请使用命令'set qbittorrent addr ip:端口'设置qb面板地址\n"
    ...
    else if (s.getContent() == "/帮助") {
        s.reply("本助手为您提供远程下载管理功能:")
        s.reply("1、您可以对我发送“查看下载”获取下载任务详情并进行管理操作")
        s.reply("2、直接发送磁链进行下载")
        s.reply("ps:本助手仅为管理员服务")
    }
    else {
...
function Login(host, name, password) {
    ...
    try {
        console.log(resp.headers["Set-Cookie"].split(";"))
        return resp.headers["Set-Cookie"].split(";").find(ele => ele.indexOf("SID") != -1)
    }
    catch (err) {
...
//暂停下载
function torrPau(host, ck, hashes) {
    console.log(hashes)
    let resp = request({
